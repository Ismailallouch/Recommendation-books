...
async function loadCriterias() {
  ...
  criterias = await res.json();
  renderCriteriaFields();
}

function renderCriteriaFields() {
  const container = document.getElementById('dynamic-criteria');
  container.innerHTML = '';
  if (!criterias.length) return;
  let row = document.createElement('div');
  row.className = 'row g-3 mt-2';
  criterias.forEach((crit, idx) => {
    const col = document.createElement('div');
    col.className = 'col-md-4';
    const label = document.createElement('label');
    label.className = 'form-label';
    label.textContent = crit.nom;
    label.htmlFor = 'crit-' + crit.nom;
    col.appendChild(label);
    const group = document.createElement('div');
    group.className = 'note-group';
    group.dataset.field = crit.nom;
    for (let i = 1; i <= 5; i++) {
      const input = document.createElement('input');
      input.type = 'radio';
      input.name = 'crit-' + crit.nom;
      input.value = i;
      input.id = 'crit-' + crit.nom + '-' + i;
...
function getCriteriaNotes() {
  return criterias.map(crit => {
    const checked = document.querySelector(`input[name='crit-${crit.nom}']:checked`);
    return { critere: crit.nom, note: checked ? Number(checked.value) : 0, obligatoire: crit.obligatoire };
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const notes = getCriteriaNotes();
      let missing = false;
      notes.forEach(n => { if (n.obligatoire && n.note < 1) missing = true; });
      if (missing) {
      ...
      const data = {
        ...
        langue: currentLang,
        notes: notes.map(n => ({ critere: n.critere, note: n.note }))
      };
      try {
